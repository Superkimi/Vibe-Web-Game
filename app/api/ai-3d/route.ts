...
import { z } from "zod";
import { aiChangeSet3DSchema, gameProject3DSchema } from "@/lib/3d-game-schema";
import { buildGameAgent3DPrompt } from "@/lib/ai-3d-prompt";

export const dynamic = "force-dynamic";

const requestSchema = z.object({
  project: gameProject3DSchema,
...
export async function POST(request: Request) {
  try {
    ...
    const prompt = buildGameAgent3DPrompt(body.project, body.message);
    ...
    try {
            ...
            messages: [
              {
                role: "system",
                content:
                  "Return only a JSON object that follows the 3D operation protocol in the user prompt.",
              },
      ...
      const changes = aiChangeSet3DSchema.parse(extractJson(modelText));
